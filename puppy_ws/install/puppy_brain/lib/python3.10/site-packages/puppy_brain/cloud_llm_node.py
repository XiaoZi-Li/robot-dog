...
特点:
  ...
  - 国内直连, 不墙
  - 支持多轮上下文 (保留最近 N 轮)
  - 订阅 /perception/result_json 缓存最近一次 YOLO 检测结果,
    当收到 tag=describe_scene 的 prompt 时, 把检测结果拼进 system prompt,
    让 LLM 能回答 "镜头里有什么东西"
"""
import json
import os
...
class CloudLlmNode(Node):
    def __init__(self):
        ...
        # 最近一次 YOLO 检测结果缓存 (供 describe_scene 提问用)
        self.last_perception = None
        self.last_perception_time = 0.0
        self.perception_topic = str(self.get_parameter('perception_topic').value)
        ...
        # 订阅 perception 结果, 缓存最近一次 (用于 "镜头里有什么" 提问)
        self.create_subscription(String, self.perception_topic, self.on_perception, 10)
    ...
    def on_prompt(self, msg: String):
        # 兼容 chat_bridge 两种 payload: 纯文本 / JSON
        try:
        ...
        self.busy = True
        threading.Thread(target=self._call_llm, args=(text, tag), daemon=True).start()

    # -----------------------------------------------------
    # 缓存最近一次 perception 结果
    # -----------------------------------------------------
    ...
    def _build_scene_text(self):
        """把缓存中的 YOLO 结果转成自然语言场景描述"""
        if not self.last_perception:
            return '当前没有识别到任何物体'

        # 2 秒以内的结果才算新鲜
        if time.time() - self.last_perception_time > 2.0:
            return '当前没有识别到任何物体 (感知数据已过期)'

        # perception_node 输出格式: {"objects": [{"label":..., "score":..., "bbox":[x1,y1,x2,y2]}], ...}
        objs = self.last_perception.get('objects') or self.last_perception.get('detections') or []
        if not objs:
            return '当前没有识别到任何物体'
        ...
        counts = {}
        for obj in objs:
            label = obj.get('label') or obj.get('class') or obj.get('name') or 'unknown'
        ...
        if len(parts) == 1:
            return f'镜头里识别到{parts[0]}'
        return f'镜头里识别到: {", ".join(parts)}'

    # -----------------------------------------------------
    # 调 DeepSeek (流式 SSE)
    # -----------------------------------------------------
    def _call_llm(self, user_text: str, tag: str = ''):
        ...
        # 构造 system prompt: 基础 + 场景描述 (仅 describe_scene 模式注入)
        sys_prompt = self.system_prompt
        if tag == 'describe_scene':
            scene = self._build_scene_text()
            sys_prompt = (
                self.system_prompt
                + f'\n\n[感知上下文] {scene}'
                + '\n用户正在问你镜头里看到了什么, 请基于上面的感知上下文用一句简洁的话回答, '
                + '不要编造感知上下文以外的物体, 不要解释技术细节.'
            )
            self.get_logger().info(f'[LLM] scene: {scene}')
        ...
        self.get_logger().info(f'[LLM] 请求(tag={tag or "-"}): "{user_text[:40]}"')
